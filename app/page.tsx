...
export default function HomePage() {
  ...
  return (
    <div className="space-y-8 sm:space-y-10">
      <section>
        <div className="overflow-hidden rounded-[28px] bg-[#152033] text-white shadow-[0_30px_80px_rgba(21,32,51,0.22)] sm:rounded-[32px]">
          <div className="soft-grid grid gap-6 bg-[radial-gradient(circle_at_top_right,_rgba(255,255,255,0.1),_transparent_28%)] p-5 sm:gap-8 sm:p-8 xl:grid-cols-[1.02fr_0.98fr]">
            <div className="flex flex-col justify-between">
              <div>
                <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/8 px-4 py-2 text-sm text-slate-200">
            ...
            <div className="grid gap-4">
              <Link
                ...
                className="block rounded-[24px] border border-white/10 bg-white/6 p-4 backdrop-blur-md transition hover:border-white/20 hover:bg-white/8 sm:rounded-[28px]"
              >
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs uppercase tracking-[0.24em] text-slate-300">Spotlight</p>
                    <p className="mt-2 text-lg font-semibold leading-snug sm:text-xl">{spotlightCase.title}</p>
                  </div>
                  <span className="shrink-0 rounded-full bg-white/10 px-3 py-1 text-xs text-slate-100">
                    {spotlightCase.result}
                  </span>
                </div>
                <div className="overflow-hidden rounded-[20px] border border-white/10 bg-[linear-gradient(180deg,_rgba(255,255,255,0.09),_rgba(255,255,255,0.03))] sm:rounded-[22px]">
                  <div className="relative flex min-h-[220px] items-center justify-center px-4 py-4 sm:min-h-[320px] sm:px-6 sm:py-6">
                    <Image
                      src={spotlightCase.images.comparisonImage ?? spotlightCase.images.plaintiffDesign ?? ""}
                      alt={spotlightCase.title}
                      width={900}
                      height={640}
                      unoptimized
                      className="mx-auto h-auto max-h-[210px] w-auto max-w-full rounded-2xl object-contain shadow-[0_12px_30px_rgba(0,0,0,0.16)] sm:max-h-[280px]"
                    />
                  </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                ...
                <div className="rounded-[24px] bg-[#f5f1e8] p-5 text-ink shadow-card">
                  ...
                  <p className="mt-1 text-3xl font-semibold">{options.industries.length}</p>
                </div>
                <div className="rounded-[24px] border border-white/10 bg-white/10 p-5 text-white shadow-[0_12px_30px_rgba(0,0,0,0.12)] sm:col-span-2 md:col-span-1">
